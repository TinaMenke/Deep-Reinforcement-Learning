        ...
        try:
            ...
            if args.record:
                self.client.start_recorder(
                    "{}/{}.log".format(os.getenv('ROOT_SCENARIO_RUNNER', "./"), config.name))
        ...
        for _, route_description in enumerate(route_descriptions_list):
            for repetition in range(repetitions):
                ...
                config = RouteScenarioConfiguration(
                    route_description, scenario_file)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--listClass', action="store_true",
                        help='List all supported scenario classes and exit')
    PARSER.add_argument(
        '--agent', help="Agent used to execute the scenario (optional). Currently only compatible with route-based scenarios.")
    PARSER.add_argument('--agentConfig', type=str,
    ...
    if not ARGUMENTS.scenario and not ARGUMENTS.route:
        print("Please specify either a scenario or use the route mode\n\n")
        PARSER.print_help(sys.stdout)
        sys.exit(0)

    if ARGUMENTS.route and ARGUMENTS.scenario:
        print("The route mode cannot be used together with a scenario'\n\n")
        PARSER.print_help(sys.stdout)
        sys.exit(0)

    if ARGUMENTS.agent and ARGUMENTS.scenario:
