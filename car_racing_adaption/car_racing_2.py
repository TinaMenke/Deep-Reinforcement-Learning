...
# These classes are needed for the preprocessing of the data
...
def crop(frame, length=12):
    ...
    """
    plt.pyplot.imsave(frame)
    h = len(frame)
    w = len(frame[0])
    return frame[0:int(h - length), int(length/2):w - int(length/2)]
...
class CarRacing2(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        self.state = _process_frame(self.state)
    ...
    def render(self, mode='human'):
        ...
        gl.glViewport(0, 0, int(VP_W), int(VP_H))
        t.enable()
        self.render_road()
...
if __name__=="__main__":
    ...
    #Stacking of five consecutive frames
    env = VecFrameStack(env,nstack=5)
    env.render()
